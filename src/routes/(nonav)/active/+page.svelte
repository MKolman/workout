<script lang="ts">
	...
	const _workout = { name: 'Loading...', work: [] };
	...
	const config = () => {
		if (!browser) return { workout: _workout, progress: _progress };
		if (!workoutId) {
			console.error('No workout specified');
			goto('/work');
			return { workout: _workout, progress: _progress };
		}
		const workout = Store.get(workoutId);
		if (!workout) {
			console.error('Workout not found');
			goto('/work');
			return { workout: _workout, progress: _progress };
		}
		const progress = getProgress(workout, programId);
		return { workout, progress };
	};
	const { workout: workout, progress } = config();
	const done = workout?.work.map((set) => set.sets.map((_) => 0));
